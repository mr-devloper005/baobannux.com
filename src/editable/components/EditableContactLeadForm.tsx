...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 rounded-[16px] border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6 md:p-8">
      ...
      {message ? (
        <div className={`mt-5 flex items-start gap-3 rounded-xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent-strong)]' : 'bg-red-50 text-red-700'}`}>
      ...
      <button
        ...
        className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-dark-bg)] px-6 text-sm font-semibold text-[var(--slot4-dark-text)] transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70"
      >
